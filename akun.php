...
<center>
  <h2 style="margin-top: 30px;">Akun Yang Sedang Digunakan</h2>
</center>
<div class="content">
  ...
  <?php
  $akun = $_SESSION['id'];
  $sql = "SELECT id, username, password, email FROM tbluser where id = '$akun'";
  $query = mysqli_query($con, $sql);
  while ($row = mysqli_fetch_array($query)) {
    ?>
    <center>
      <br>
      <a href="editpassword.php?id=<?php echo $row["id"]; ?>" class="button">
        Edit password
      </a>
    </center>
    <?php
...
<?php
// mengambil data profil dari tabel accounts
$stmt = $con->prepare('SELECT namalengkap, nomor, alamat FROM tbluser WHERE id = ?');
// mengambil data profil berdasarkan id
$stmt->bind_param('i', $_SESSION['id']);
$stmt->execute();
$stmt->bind_result($namalengkap, $nomor, $alamat);
$stmt->fetch();
$stmt->close();
?>
...
<div class="content">
  ...
  while ($row = mysqli_fetch_array($query)) {
    ...
    <br>
    <center>
      <a href="editprofil.php?id=<?php echo $row["id"]; ?>" class="button">
        Edit Profil
      </a>
    </center>
    <?php
